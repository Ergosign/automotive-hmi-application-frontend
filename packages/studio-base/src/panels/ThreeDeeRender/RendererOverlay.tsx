...
// import { InteractionContextMenu, SelectionObject } from "./Interactions";
...
export function RendererOverlay(props: {
  ...
  const { setCurrentPanelInterface, setCurrentPanelActionComponent, setCanResetView } =
    usePanelActionsStore();
  const analytics = useAnalytics();
  const { enqueueSnackbar } = useSnackbar();
  const { t } = useTranslation("panels");
  const { classes } = useStyles();
  const [_clickedPosition, setClickedPosition] = useState<{ clientX: number; clientY: number }>({
    clientX: 0,
    clientY: 0,
  });
  const [_selectedRenderables, setSelectedRenderables] = useState<PickedRenderable[]>([]);
  ...
  const [_showResetViewButton, setShowResetViewButton] = useState(
    renderer?.canResetView() ?? false,
  );
  useRendererEvent(
    "resetViewChanged",
    useCallback(() => {
      const canResetView = renderer?.canResetView() ?? false;

      setShowResetViewButton(canResetView);
      setCanResetView(props.panelId, canResetView);
    }, [props.panelId, renderer, setCanResetView]),
  );
  ...
  // const publicClickButtonRef = useRef<HTMLButtonElement>(ReactNull);
  ...
  //         <Menu
  //           id="publish-menu"
  //           anchorEl={publicClickButtonRef.current}
  ...
  // Mounting Panel actions to zustand store to be able to use it on different parts of the application.
  useEffect(() => {
    setCurrentPanelInterface(props.interfaceMode);
    // if (props.interfaceMode === "3d") {
    setCurrentPanelActionComponent(
      <div className={classes.threeDeePanelViews}>
        <span>{t("3DPanel")}</span>
        <div className={classes.iconButtonContainer}>
          <IconButton
            className={`${classes.iconButton} ${props.perspective ? classes.iconButtonActive : ""}`}
            title={props.perspective ? "Switch to 2D camera" : "Switch to 3D camera"}
            onClick={!props.perspective ? props.onTogglePerspective : undefined}
          >
            <ThreeDRotationIcon />
          </IconButton>
          <IconButton
            className={`${classes.iconButton} ${
              !props.perspective ? classes.iconButtonActive : ""
            }`}
            title={props.perspective ? "Switch to 2D camera" : "Switch to 3D camera"}
            onClick={props.perspective ? props.onTogglePerspective : undefined}
          >
    ...
    // } else {
    //   setCurrentPanelActionComponent(undefined);
    // }
